...
COMMENT ON COLUMN rail_day_plan.platform_risk IS '站台风险标记：0 无风险 / 1 PLATFORM_RISK（站台长度下调后编组超长）';
...
COMMENT ON TABLE rail_plan_consist IS '计划编组登记/替换的最新一行';
...
COMMENT ON COLUMN rail_plan_consist.platform_code IS '登记停靠站台代码，必须存在';
...
COMMENT ON COLUMN rail_plan_consist_car.plan_id IS '所属计划 id';
...
COMMENT ON TABLE rail_platform_risk_snapshot IS '站台风险快照，标记 PLATFORM_RISK 时固化，整改清除后可再次标记';
COMMENT ON COLUMN rail_platform_risk_snapshot.plan_id IS '受影响计划 id，唯一';

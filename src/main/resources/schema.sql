...
CREATE TABLE IF NOT EXISTS race (
    ...
    status VARCHAR(16) NOT NULL COMMENT '赛事状态：OPEN-开放可写，SUSPENDED-已中止仅允许恢复，SEALED-已封榜只读',
...
CREATE TABLE IF NOT EXISTS result_snapshot_entry (
    ...
    net_finish_time_ms BIGINT COMMENT '净完赛耗时=原始完赛扣除中止补偿（毫秒）；计时缺失为NULL',
    net_total_time_ms BIGINT COMMENT '净总耗时=净完赛+生效加时（毫秒），排名依据；未排名时为NULL',
...
CREATE TABLE IF NOT EXISTS result_snapshot_checkpoint (
    ...
    net_elapsed_millis BIGINT COMMENT '封榜时该选手通过该检查点的净累计耗时=原始分段扣除中止补偿（毫秒）；缺失检查点为NULL',
...
CREATE TABLE IF NOT EXISTS suspension_event (
    event_key VARCHAR(64) NOT NULL COMMENT '中止事件Key，全局唯一',
    race_id VARCHAR(64) NOT NULL COMMENT '所属赛事ID',
    checkpoint_code VARCHAR(64) NOT NULL COMMENT '受影响起始检查点代码',
    checkpoint_position INT NOT NULL COMMENT '受影响起始检查点顺序（登记时固化）',
    start_elapsed_ms BIGINT NOT NULL COMMENT '中止开始累计耗时（毫秒，1~86400000）',
    resume_elapsed_ms BIGINT COMMENT '恢复累计耗时（毫秒），必须大于start_elapsed_ms；未恢复为NULL',
    status VARCHAR(16) NOT NULL COMMENT '事件状态：SUSPENDED-中止中，RESUMED-已恢复',
    created_at BIGINT NOT NULL COMMENT '登记时间，Unix毫秒时间戳',
    resumed_at BIGINT COMMENT '恢复时间，Unix毫秒时间戳；未恢复为NULL',
...
CREATE TABLE IF NOT EXISTS result_snapshot_suspension (
    race_id VARCHAR(64) NOT NULL COMMENT '所属快照的赛事ID',
    event_key VARCHAR(64) NOT NULL COMMENT '中止事件Key',
    checkpoint_code VARCHAR(64) NOT NULL COMMENT '受影响起始检查点代码',
    start_elapsed_ms BIGINT NOT NULL COMMENT '中止开始累计耗时（毫秒）',
    resume_elapsed_ms BIGINT NOT NULL COMMENT '恢复累计耗时（毫秒）',

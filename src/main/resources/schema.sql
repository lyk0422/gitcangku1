...
CREATE TABLE IF NOT EXISTS incidents (
    ...
    containment_deadline TIMESTAMP(6) NULL COMMENT '遏制期限 UTC：首次进入 COMMANDING 时按该次接管时刻与等级确定（S1+5分钟/S2+15分钟/S3+60分钟/S4+240分钟），交接不重置；REPORTED 状态为空',
...
CREATE TABLE IF NOT EXISTS incident_escalations (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '自增主键',
    incident_id BIGINT NOT NULL COMMENT '所属事件 id，关联 incidents.id；每个事件至多一条升级记录',
    status VARCHAR(16) NOT NULL COMMENT '状态：OPEN 待确认 / ACKNOWLEDGED 已确认 / CANCELLED 已取消（遏制时仍 OPEN 原子置此态）',
    deadline TIMESTAMP(6) NOT NULL COMMENT '遏制期限 UTC：首次接管时刻 + 等级时长（S1 5分钟/S2 15分钟/S3 60分钟/S4 240分钟），交接不重置',
    triggered_at TIMESTAMP(6) NOT NULL COMMENT '检查命中逾期的触发 UTC 时刻；保存当时的期限评估结果',
    commander VARCHAR(128) NOT NULL COMMENT '触发升级时的当前指挥人；交接后该字段保留为触发时指挥人',
    disposition_note VARCHAR(1024) NULL COMMENT '确认时提交的非空处置说明；仅 ACKNOWLEDGED 有值，否则为空',
    acknowledged_by VARCHAR(128) NULL COMMENT '确认人（确认操作当时的当前指挥人）；仅 ACKNOWLEDGED 有值，否则为空',

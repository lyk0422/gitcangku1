...
CREATE TABLE IF NOT EXISTS maintenance (
    ...
    deduction_total_minutes BIGINT NOT NULL,
    request_id VARCHAR(128) NOT NULL,
...
COMMENT ON COLUMN maintenance.run_minutes IS '保养完成时固化的本轮运行分钟（已扣减生效停机区间，为负按 0 计）';
COMMENT ON COLUMN maintenance.deduction_total_minutes IS '保养完成时固化的本次结算扣减合计（上一锚点之后全部生效停机区间扣减量之和，分钟）';
COMMENT ON COLUMN maintenance.request_id IS '完成保养请求的 requestId';
COMMENT ON COLUMN maintenance.completed_at IS '保养完成登记时刻（UTC）';

-- 停机区间：左闭右开 [start_at, end_at)；同设备生效（ACTIVE）区间不得相互重叠（端点相接合法）。
-- 撤销后记录保留且不可改写，扣减量固化于撤销前最后一次重算值，不再参与运行分钟计算。
CREATE TABLE IF NOT EXISTS downtime (
    downtime_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    downtime_key VARCHAR(128) NOT NULL,
    equipment_id VARCHAR(64) NOT NULL,
    ...
    CONSTRAINT uq_downtime_key UNIQUE (downtime_key)
);
CREATE INDEX IF NOT EXISTS idx_downtime_equipment ON downtime (equipment_id, start_at);
COMMENT ON TABLE downtime IS '设备停机区间：左闭右开；ACTIVE 参与扣减，REVOKED 保留快照不再参与';
COMMENT ON COLUMN downtime.downtime_id IS '停机记录自增主键';
COMMENT ON COLUMN downtime.downtime_key IS '停机业务键，全局唯一（含已撤销记录，不可复用）';
COMMENT ON COLUMN downtime.equipment_id IS '所属设备标识';
COMMENT ON COLUMN downtime.start_at IS '停机开始时刻（UTC，含）；须不早于设备最早读数采样时刻';
COMMENT ON COLUMN downtime.end_at IS '停机结束时刻（UTC，不含）；须晚于开始且不晚于设备最晚读数采样时刻';
COMMENT ON COLUMN downtime.reason IS '停机原因，非空';
COMMENT ON COLUMN downtime.deduction_minutes IS '区间扣减量（分钟）：不晚于结束时刻的最近读数累计分钟减去不晚于开始时刻的最近读数累计分钟，取不到为 0；读数新增/修订时重算，撤销后固化';
COMMENT ON COLUMN downtime.status IS '状态：ACTIVE=生效，REVOKED=已撤销（不再参与扣减，记录不可改写）';
...
COMMENT ON COLUMN downtime.revoke_request_id IS '撤销请求的 requestId，未撤销为 NULL';

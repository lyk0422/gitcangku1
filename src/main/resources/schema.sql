...
CREATE TABLE IF NOT EXISTS device (
  ...
  version INT NOT NULL DEFAULT 1 COMMENT '设备配置版本号，从1开始，每次维护窗口修改成功加一',
  utc_offset_minutes INT NULL COMMENT '设备本地时区相对UTC的偏移分钟，取值-720~840；NULL表示未配置维护窗口',
  window_start_minute INT NULL COMMENT '每日维护窗口本地起始分钟，取值0~1439，左闭；与结束分钟不同，大于结束分钟表示跨零点',
  window_end_minute INT NULL COMMENT '每日维护窗口本地结束分钟，取值0~1439，右开',
...
CREATE TABLE IF NOT EXISTS release_order (
  ...
  respect_maintenance_window BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否要求设备拉取落在其本地维护窗口内；FALSE保持原有拉取行为',
...
CREATE TABLE IF NOT EXISTS task_deferral (
  id BIGINT NOT NULL AUTO_INCREMENT COMMENT '顺延记录ID',
  release_id BIGINT NOT NULL COMMENT '所属发布单ID',
  device_id VARCHAR(64) NOT NULL COMMENT '设备ID',
  defer_count INT NOT NULL DEFAULT 0 COMMENT '该设备在该发布单下累计被顺延次数，人工恢复不清零',
  last_deferred_at_utc VARCHAR(40) NOT NULL COMMENT '最近顺延时刻，UTC，ISO-8601格式（如2026-09-24T07:00:00Z）',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最近变更时间',
  PRIMARY KEY (id),
  CONSTRAINT uk_deferral_release_device UNIQUE (release_id, device_id)
) COMMENT='设备拉取顺延统计，同设备同发布单最多一条，历史不清零';

...
CREATE TABLE IF NOT EXISTS result_snapshot_entry (
    ...
    status VARCHAR(24) NOT NULL COMMENT '成绩状态：RANKED-参与排名，UNTIMED-计时缺失，MISSING_CHECKPOINT-有完赛但未覆盖全部检查点，DISQUALIFIED-取消资格，DNS-未出发退赛，DNF-中途退赛',
...
CREATE TABLE IF NOT EXISTS withdrawal (
    ...
    status VARCHAR(8) NOT NULL COMMENT '退赛状态：DNS-未出发，DNF-中途退赛；DNS与DNF不允许相互改写',
    reason VARCHAR(512) NOT NULL COMMENT '退赛原因，非空',
    last_checkpoint_code VARCHAR(64) COMMENT 'DNF登记时最后通过的检查点代码（须为该选手已有分段记录中顺序最大者）；DNS为NULL',
    revoked BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否已撤销：FALSE-生效中，TRUE-已撤销；撤销记录不可变，已撤销不能再次撤销',

...
COMMENT ON COLUMN rail_disruption_switch.switch_key IS '切换单业务键，全局唯一，换请求复用同一键判定 409';
...
COMMENT ON COLUMN rail_disruption_switch.activated_request_id IS '成功激活的请求 ID（requestId），激活前为空，用于区分同切换单的重放与换请求复用';
...
COMMENT ON TABLE rail_disruption_mapping IS '切换单提交的旧计划到替代草稿计划的一对一映射，提交后不可改写';

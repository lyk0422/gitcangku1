...
CREATE TABLE IF NOT EXISTS consent_record (
    ...
    scope_key VARCHAR(128) NOT NULL DEFAULT 'default' COMMENT '所属子范围标识，default 表示未指定子范围的默认子范围',
...
-- 授权子范围表：同一有效代次内的逻辑分区，不产生新代次；只允许从有效变为已撤回
CREATE TABLE IF NOT EXISTS consent_scope (
    subject_key VARCHAR(128) NOT NULL COMMENT '主体标识（合成字符串）',
    purpose VARCHAR(32) NOT NULL COMMENT '用途：RESEARCH 研究 / PERSONALIZATION 个性化',
    epoch INT NOT NULL COMMENT '子范围所属授权代次',
    scope_key VARCHAR(128) NOT NULL COMMENT '子范围标识，同一 epoch 内唯一；default 为保留值',
    label VARCHAR(256) NOT NULL COMMENT '子范围非空标签（合成字符串）',
    status VARCHAR(16) NOT NULL COMMENT '状态：ACTIVE 有效 / REVOKED 已撤回',
...
CREATE TABLE IF NOT EXISTS idempotency_request (
    ...
    operation VARCHAR(32) NOT NULL COMMENT '操作类型：GRANT 授权 / WRITE 写入 / REVOKE 撤回 / SCOPE_CREATE 子范围创建 / SCOPE_REVOKE 子范围撤回',

...
CREATE TABLE IF NOT EXISTS result_snapshot_entry (
    ...
    status VARCHAR(24) NOT NULL COMMENT '参赛状态：RANKED-参与排名，UNTIMED-计时缺失，MISSING_CHECKPOINT-有完赛但未覆盖全部检查点，DISQUALIFIED-取消资格，DNS-未出发，DNF-中途退赛',
    ...
    last_checkpoint_code VARCHAR(64) COMMENT '封榜时该选手最后通过（顺序最大）的检查点代码；无任何分段记录为NULL',
...
CREATE TABLE IF NOT EXISTS withdrawal (
    withdrawal_key VARCHAR(128) NOT NULL COMMENT '退赛登记键，全局唯一（第二层幂等键）',
    race_id VARCHAR(64) NOT NULL COMMENT '所属赛事ID',
    bib VARCHAR(64) NOT NULL COMMENT '退赛选手参赛号',
    status VARCHAR(8) NOT NULL COMMENT '退赛状态：DNS-未出发，DNF-中途退赛',
    reason VARCHAR(512) NOT NULL COMMENT '退赛原因，非空',
    last_checkpoint_code VARCHAR(64) COMMENT 'DNF登记的最后通过检查点代码（须为该选手已有分段中顺序最大者）；DNS为NULL',
    revoked BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否已撤销：FALSE-生效中，TRUE-已撤销；撤销记录不可变',
    created_at BIGINT NOT NULL COMMENT '退赛登记时间，Unix毫秒时间戳',
    revoked_at BIGINT COMMENT '退赛撤销时间，Unix毫秒时间戳；未撤销为NULL',
    CONSTRAINT pk_withdrawal PRIMARY KEY (withdrawal_key),
...
CREATE TABLE IF NOT EXISTS idempotency_record (
    ...
    operation VARCHAR(48) NOT NULL COMMENT '操作类型：CREATE_RACE/REGISTER_RUNNER/REVISE_TIME/ADD_PENALTY/REVOKE_PENALTY/CONFIGURE_CHECKPOINTS/SUBMIT_TIMING/SEAL_RACE/REGISTER_WITHDRAWAL/REVOKE_WITHDRAWAL',

...
-- 所有时间字段为数据库默认时区时间戳；版本号均从 1（文档发布版本、术语版本从 0）开始单调递增。
...
COMMENT ON COLUMN document.draft_version IS '文档草稿版本，从 1 开始；增段落或修改源文/译文、新增术语版本时加一';
COMMENT ON COLUMN document.published_version IS '已发布版本号，从 0 开始，每次成功发布加一';
COMMENT ON COLUMN document.term_version IS '当前术语版本，从 0（尚无术语表）开始，每次新增不可变术语快照加一';
...
COMMENT ON COLUMN translation.term_version IS '提交译文时绑定的术语版本；落后于文档当前术语版本即术语过期，须重新提交';
...
COMMENT ON TABLE term_version IS '术语版本：文档内从 1 开始单调递增的不可变术语快照；已有版本不可覆盖';
COMMENT ON COLUMN term_version.document_id IS '所属文档 ID';
COMMENT ON COLUMN term_version.term_version IS '术语版本号，从 1 开始；0 保留为文档尚无术语表的初始状态，不在此表';
COMMENT ON COLUMN term_version.created_at IS '术语版本创建时间，数据库默认时区';
...
CREATE TABLE IF NOT EXISTS term_rule (
    ...
    required_translation LONGTEXT NOT NULL,
    rule_order INT NOT NULL,
    PRIMARY KEY (document_id, term_version, language, source_term)
);
COMMENT ON TABLE term_rule IS '术语规则：属于某术语版本的不可变快照，按目标语言与 sourceTerm 唯一，0~100 条';
COMMENT ON COLUMN term_rule.document_id IS '所属文档 ID';
COMMENT ON COLUMN term_rule.term_version IS '所属术语版本号';
COMMENT ON COLUMN term_rule.language IS '规则目标语言码，小写；仅校验该语言的译文';
COMMENT ON COLUMN term_rule.source_term IS '术语原文，UTF-8，非空；按 Unicode 原文、区分大小写做连续子串匹配';
COMMENT ON COLUMN term_rule.required_translation IS '要求译文中必须出现的非空译文，连续子串匹配';
COMMENT ON COLUMN term_rule.rule_order IS '规则在提交规则集中的顺序，从 0 开始，用于结果稳定排序';
...
COMMENT ON COLUMN release_snapshot.snapshot_json IS '快照内容 JSON：全部段落源文及各语言译文、作者、审核人、版本号，以及固化的术语版本与实际规则集';

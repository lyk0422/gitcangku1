...
CREATE TABLE IF NOT EXISTS leg (
    ...
    max_bags        INT          NOT NULL COMMENT '航段装载件数上限，取值 1~500（件）',
    max_weight      INT          NOT NULL COMMENT '航段装载总重上限，取值 1~50000（千克）',
...
CREATE TABLE IF NOT EXISTS bag (
    ...
    status              VARCHAR(16)  NOT NULL DEFAULT 'IN_TRANSIT' COMMENT '行李状态：IN_TRANSIT 在途/SHORT_UNLOADED 短卸待补/OFFLOADED 容量卸载待改派/RECOVERED 已补到在途/DELIVERED 已交付',
    weight              INT          NOT NULL COMMENT '行李重量，取值 1~50（千克，整数）',
    cabin               VARCHAR(16)  NOT NULL COMMENT '舱位等级：PREMIUM/STANDARD/BASIC，卸载优先级 BASIC 先于 STANDARD 先于 PREMIUM',
...
-- 容量卸载决策：offload_key 全局唯一；记录目标上限与卸载后保留占用，随业务事务提交
CREATE TABLE IF NOT EXISTS offload_record (
    offload_key        VARCHAR(128) NOT NULL COMMENT '全局唯一卸载请求标识，同键重放原结果、异键互不影响',
    leg_id             VARCHAR(64)  NOT NULL COMMENT '卸载发生的航段标识',
    target_max_bags    INT          NOT NULL COMMENT '卸载决策的目标件数上限（件）',
    target_max_weight  INT          NOT NULL COMMENT '卸载决策的目标总重上限（千克）',
    retained_count     INT          NOT NULL COMMENT '卸载后继续保留在航段清单的件数',
    retained_weight    INT          NOT NULL COMMENT '卸载后继续保留在航段清单的总重（千克）',
    offloaded_at       TIMESTAMP WITH TIME ZONE NOT NULL COMMENT '卸载执行时刻（UTC）',
    created_at         TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (offload_key)
);

-- 容量卸载逐件明细：记录被卸行李、原待乘索引（卸载不推进）与 UTC 卸载时刻
CREATE TABLE IF NOT EXISTS offload_item (
    id                  BIGINT AUTO_INCREMENT NOT NULL COMMENT '卸载明细自增主键',
    offload_key         VARCHAR(128) NOT NULL COMMENT '所属卸载决策标识',
    leg_id              VARCHAR(64)  NOT NULL COMMENT '卸载航段标识',
    bag_tag             VARCHAR(64)  NOT NULL COMMENT '被卸行李牌号',
    weight              INT          NOT NULL COMMENT '被卸行李重量（千克）',
    cabin               VARCHAR(16)  NOT NULL COMMENT '被卸行李舱位：PREMIUM/STANDARD/BASIC',
    origin_next_leg_index INT        NOT NULL COMMENT '卸载时该行李的原待乘航段下标（0 起），卸载不推进',
    offloaded_at        TIMESTAMP WITH TIME ZONE NOT NULL COMMENT '卸载时刻（UTC）',
    PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS bag_event (
    ...
    event_type VARCHAR(32)  NOT NULL COMMENT '事件类型：REGISTERED 登记/LOADED 装载/UNLOADED 到达卸下/SHORT_UNLOADED 短卸/CAPACITY_OFFLOADED 容量卸载/RECOVERED 补到/DELIVERED 交付',
...
CREATE TABLE IF NOT EXISTS request_log (
    ...
    operation       VARCHAR(32)  NOT NULL COMMENT '操作类型：REGISTER_LEG/REGISTER_BAG/LOAD/SEAL/ARRIVE/ARRIVE_DIFFERENCE/RECOVER/OFFLOAD',

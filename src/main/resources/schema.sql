...
-- 保养延期记录：申请即写入，批准后不可变；仅作用于申请时所处的保养周期。
CREATE TABLE IF NOT EXISTS deferral (
    ...
    cycle_maintenance_id BIGINT NOT NULL,
    ...
    applied_run_minutes BIGINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_deferral_equipment ON deferral (equipment_id, cycle_maintenance_id);
COMMENT ON TABLE deferral IS '保养延期记录：PENDING/APPROVED/REJECTED/EXPIRED；批准后阈值快照不可变';
COMMENT ON COLUMN deferral.deferral_id IS '延期记录自增主键';
COMMENT ON COLUMN deferral.equipment_id IS '所属设备标识';
COMMENT ON COLUMN deferral.defer_key IS '延期申请标识，设备内唯一（含历史周期，不可复用）';
COMMENT ON COLUMN deferral.cycle_maintenance_id IS '申请时所处保养周期标识：最近保养记录主键，无保养时为 0；完成保养后该周期延期额度归零';
COMMENT ON COLUMN deferral.requested_minutes IS '申请延期分钟数，1～10080，且不超过保养周期的 25%';
...
COMMENT ON COLUMN deferral.applicant IS '申请人标识';
COMMENT ON COLUMN deferral.approver IS '审批人标识（批准或拒绝的操作人），待审批时为 NULL';
COMMENT ON COLUMN deferral.reject_reason IS '拒绝理由，仅 REJECTED 时有值';
COMMENT ON COLUMN deferral.applied_run_minutes IS '申请时刻本轮运行分钟（最新读数工时 - 最近保养锚点工时）';
COMMENT ON COLUMN deferral.original_threshold_minutes IS '批准时原阈值（分钟）= 保养周期 + 本周期此前已批准延期；待审批/已拒绝时为 NULL';
COMMENT ON COLUMN deferral.new_threshold_minutes IS '批准后新阈值（分钟）= 原阈值 + 本次延期；待审批/已拒绝时为 NULL';
COMMENT ON COLUMN deferral.request_id IS '产生该申请的请求 requestId';
COMMENT ON COLUMN deferral.created_at IS '申请时刻（UTC）';
COMMENT ON COLUMN deferral.decided_at IS '审批/失效时刻（UTC），待审批时为 NULL';

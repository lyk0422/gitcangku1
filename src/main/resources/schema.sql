...
CREATE TABLE IF NOT EXISTS course (
    ...
    current_record_id VARCHAR(64) COMMENT '当前纪录ID（指向 course_record.record_id）；NULL表示尚无纪录',
...
CREATE TABLE IF NOT EXISTS race (
    ...
    course_key VARCHAR(64) NOT NULL COMMENT '所属赛道标识，创建时关联且不可变更',
    ...
    INDEX idx_race_course (course_key)
);
...
CREATE TABLE IF NOT EXISTS course_record (
    record_id VARCHAR(64) NOT NULL COMMENT '纪录ID，全局唯一，认定通过时生成',
    claim_key VARCHAR(64) NOT NULL COMMENT '认定申请键 recordClaimKey，全局唯一；重复申请按此键幂等返回首次结果',
    course_key VARCHAR(64) NOT NULL COMMENT '所属赛道标识',
    seq INT NOT NULL COMMENT '历史链内序号，从1开始，每次纪录切换加一；链只增长不可删改',
    race_id VARCHAR(64) NOT NULL COMMENT '产生该纪录的赛事ID',
    bib VARCHAR(64) NOT NULL COMMENT '创纪录选手参赛号',
    time_ms BIGINT NOT NULL COMMENT '纪录计时（封榜快照最终计时=原始完赛耗时+生效加时，毫秒）',
    claimed_at BIGINT NOT NULL COMMENT '纪录认定切换时刻，Unix毫秒时间戳',
    CONSTRAINT pk_course_record PRIMARY KEY (record_id),
    CONSTRAINT uk_course_record_claim UNIQUE (claim_key),
    CONSTRAINT uk_course_record_course_seq UNIQUE (course_key, seq),
...
CREATE TABLE IF NOT EXISTS idempotency_record (
    ...
    operation VARCHAR(48) NOT NULL COMMENT '操作类型：CREATE_RACE/REGISTER_RUNNER/REVISE_TIME/ADD_PENALTY/REVOKE_PENALTY/SEAL_RACE/REGISTER_COURSE/CLAIM_RECORD',

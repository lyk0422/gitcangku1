...
CREATE TABLE IF NOT EXISTS leg (
    ...
    max_pieces      INT          NOT NULL COMMENT '件数上限（1~500），装载后件数不得超过',
    max_weight_kg   INT          NOT NULL COMMENT '总重上限（千克，1~50000），装载后总重不得超过',
...
CREATE TABLE IF NOT EXISTS bag (
    ...
    next_leg_index      INT          NOT NULL DEFAULT 0 COMMENT '待乘航段在行程中的下标（0 起），等于行程长度表示已完成；短卸/卸载不推进',
    status              VARCHAR(16)  NOT NULL DEFAULT 'IN_TRANSIT' COMMENT '行李状态：IN_TRANSIT 在途/SHORT_UNLOADED 短卸待补/OFFLOADED 超载卸载待改派/RECOVERED 已补到在途/DELIVERED 已交付',
    weight_kg           INT          NOT NULL COMMENT '行李重量（千克，整数 1~50）',
    cabin_class         VARCHAR(8)   NOT NULL COMMENT '舱位等级：PREMIUM/STANDARD/BASIC，卸载优先级 BASIC 先于 STANDARD 先于 PREMIUM',
    ...
    offload_leg_id      VARCHAR(64)  NULL COMMENT '卸载航段标识，仅 OFFLOADED 状态非 NULL',
    offload_next_index  INT          NULL COMMENT '被卸时的原待乘索引（0 起），卸载不推进，仅 OFFLOADED 状态非 NULL',
    offloaded_at        TIMESTAMP WITH TIME ZONE NULL COMMENT '卸载时刻（UTC），仅 OFFLOADED 状态非 NULL',
    created_at          TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (bag_tag)
);
...
-- 卸载明细：同一次卸载决策（同一 offload_key）可含多行，(offload_key, bag_tag) 唯一
CREATE TABLE IF NOT EXISTS offload_record (
    id                 BIGINT AUTO_INCREMENT NOT NULL COMMENT '自增主键',
    offload_key        VARCHAR(128) NOT NULL COMMENT '卸载决策幂等键，全局唯一，同一次决策的多行共用',
    leg_id             VARCHAR(64)  NOT NULL COMMENT '被卸航段标识',
    bag_tag            VARCHAR(64)  NOT NULL COMMENT '被卸行李牌号',
    origin_index       INT          NOT NULL COMMENT '被卸时的原待乘索引（0 起），卸载不推进',
    weight_kg          INT          NOT NULL COMMENT '被卸行李重量（千克）',
    cabin_class        VARCHAR(8)   NOT NULL COMMENT '被卸行李舱位等级：PREMIUM/STANDARD/BASIC',
    target_max_pieces  INT          NOT NULL COMMENT '该次决策目标件数上限',
    target_max_weight_kg INT       NOT NULL COMMENT '该次决策目标总重上限（千克）',
    offloaded_at       TIMESTAMP WITH TIME ZONE NOT NULL COMMENT '卸载执行时刻（UTC）',
    PRIMARY KEY (id),
    UNIQUE (offload_key, bag_tag)
);
...
CREATE TABLE IF NOT EXISTS bag_event (
    ...
    event_type VARCHAR(32)  NOT NULL COMMENT '事件类型：REGISTERED 登记/LOADED 装载/UNLOADED 到达卸下/OFFLOADED 超载卸载/SHORT_UNLOADED 短卸/RECOVERED 补到或改派/DELIVERED 交付',
...
-- 幂等去重：仅记录成功请求；同 requestId/offloadKey 同参数重放原结果，异参数返回 409
CREATE TABLE IF NOT EXISTS request_log (
    request_id      VARCHAR(128) NOT NULL COMMENT '全局唯一请求标识（卸载决策为 offloadKey）',
    operation       VARCHAR(32)  NOT NULL COMMENT '操作类型：REGISTER_LEG/REGISTER_BAG/LOAD/SEAL/OFFLOAD/ARRIVE/ARRIVE_DIFFERENCE/RECOVER',
    request_hash    VARCHAR(64)  NOT NULL COMMENT '请求参数（不含幂等键）的 SHA-256 摘要',

...
COMMENT ON TABLE fallback_config IS '回退语种配置：每个目标语种至多一个回退语种，构成有向无环链；整表按文档全量替换';
COMMENT ON COLUMN fallback_config.document_id IS '所属文档 ID';
COMMENT ON COLUMN fallback_config.language IS '目标语言码，小写；文档内唯一';
COMMENT ON COLUMN fallback_config.fallback_language IS '回退目标语言码，小写，须已登记为文档目标语言且不得成环';
COMMENT ON COLUMN fallback_config.updated_at IS '最近配置时间，数据库默认时区';

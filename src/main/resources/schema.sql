...
-- 脚本幂等：新库直接建成最新结构；历史库执行末尾的兼容迁移语句。
...
COMMENT ON COLUMN equipment.maintenance_period_minutes IS 'DEFAULT 项目的保养周期（分钟），正整数，登记后不可修改';
COMMENT ON COLUMN equipment.version IS '设备版本号，初始 1；新增项目/新增读数/修订/任一项目完成保养均校验 expectedVersion 并加一';
COMMENT ON COLUMN equipment.created_at IS '登记时刻（UTC）';

-- 保养项目表：每台设备登记时自动生成 itemCode=DEFAULT 的项目（周期取设备登记周期），
-- 之后可新增最多 20 个自定义项目；itemCode 设备内唯一，项目创建后周期不可修改、不可删除。
...
COMMENT ON TABLE maintenance_item IS '保养项目：DEFAULT 为登记时迁移的原项目，其余为新增项目，创建后不可修改或删除';
COMMENT ON COLUMN maintenance_item.equipment_id IS '所属设备标识';
COMMENT ON COLUMN maintenance_item.item_code IS '项目编码，设备内唯一；DEFAULT 为登记时原周期项目，自定义项目最多 20 个';
COMMENT ON COLUMN maintenance_item.maintenance_period_minutes IS '该项目独立保养周期（分钟），正整数，创建后不可修改';
COMMENT ON COLUMN maintenance_item.created_at IS '项目创建时刻（UTC）；DEFAULT 项目与设备同时刻迁移生成';

-- 工时读数当前值表：reading_id 设备内唯一，同设备同一采样时刻仅一条。所有保养项目共享同一组读数。
...
-- 保养记录：每个项目独立一行，锚定某读数的特定修订号，保存锚点时刻与工时快照，不允许删除。
-- 同一读数可作为不同项目的锚点；只要读数被至少一个项目的记录引用，该读数即不可修订。
CREATE TABLE IF NOT EXISTS maintenance (
    maintenance_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    equipment_id VARCHAR(64) NOT NULL,
    item_code VARCHAR(64) NOT NULL DEFAULT 'DEFAULT',
    reading_id VARCHAR(64) NOT NULL,
...
-- 历史库兼容：旧 maintenance 表缺少 item_code 列，此处幂等补列（新库建表时已含，操作为空操作）。
ALTER TABLE maintenance ADD COLUMN IF NOT EXISTS item_code VARCHAR(64) NOT NULL DEFAULT 'DEFAULT';
...
-- 历史数据迁移：既有保养记录归属 DEFAULT 项目；为缺少 DEFAULT 项目的设备按其登记周期补建项目。
UPDATE maintenance SET item_code = 'DEFAULT' WHERE item_code IS NULL OR item_code = '';
INSERT INTO maintenance_item (equipment_id, item_code, maintenance_period_minutes, created_at)
SELECT e.equipment_id, 'DEFAULT', e.maintenance_period_minutes, e.created_at
FROM equipment e
WHERE NOT EXISTS (
    SELECT 1 FROM maintenance_item mi
    WHERE mi.equipment_id = e.equipment_id AND mi.item_code = 'DEFAULT'
);
COMMENT ON TABLE maintenance IS '项目保养记录：同项目锚点时间须严格晚于上次；任一项目锚定的读数即不可修订；记录不允许删除';
COMMENT ON COLUMN maintenance.maintenance_id IS '保养记录自增主键';
COMMENT ON COLUMN maintenance.equipment_id IS '所属设备标识';
COMMENT ON COLUMN maintenance.item_code IS '保养项目编码；DEFAULT 为原周期项目';
COMMENT ON COLUMN maintenance.reading_id IS '锚点读数标识（同一读数可被不同项目引用）';

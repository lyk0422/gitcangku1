...
public class ReleaseRepository {
    ...
    private static final String COLUMNS = "id, version, model, from_version, to_version, ratio, status,"
            + " sample_floor, failure_threshold_percent, respect_maintenance_window, monitor_round,"
        ...
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO release_order (version, model, from_version, to_version, ratio, status,"
                            + " sample_floor, failure_threshold_percent, respect_maintenance_window,"
                            + " monitor_round, active_model)"
                            + " VALUES (1, ?, ?, ?, ?, 'ACTIVE', ?, ?, ?, 1, ?)",
                    new String[]{"id"});
            ...
            ps.setBoolean(7, respectMaintenanceWindow);

...
public final class ApiDtos {
    ...
    /** 术语规则输入：sourceTerm 区分大小写；suppressed 为 true 时抑制对应全局规则且不得携带必译文本。 */
    ...
    /** 全局术语库引用升级请求：携带文档当前引用的全局版本与当前草稿版本两个期望版本。 */
    public record UpgradeGlossaryRequest(
            @NotBlank(message = "requestId 不能为空") @Size(max = 128) String requestId,
            @PositiveOrZero(message = "expectedGlobalTermVersion 不能为负数") int expectedGlobalTermVersion,
            @Positive(message = "expectedDraftVersion 必须为正数") int expectedDraftVersion) {
    }
    ...
    /** 全局术语库引用升级响应：升级后引用的全局版本与加一后的草稿版本。 */
    public record GlossaryUpgradeResponse(long documentId, int globalTermVersion, int draftVersion) {
    }

    /** 生效术语规则视图：合成后的规则及其来源（GLOBAL/DOCUMENT/SUPPRESSED）。 */
    public record EffectiveTermRuleView(String sourceTerm, String language, String requiredTranslation,
                                        String source) {
    }

    /** 生效规则集查询视图：文档当前引用的两个术语版本与合成后的完整规则集。 */
    public record EffectiveTermsView(long documentId, int termVersion, int globalTermVersion,
    ...
    /** 术语规则视图：suppressed 为 true 时表示该规则抑制对应全局规则，requiredTranslation 为 null。 */
    ...
    /** 单条译文的术语状态：绑定版本、是否过期及当前生效规则下的违规术语。 */
    ...
    /** 译文术语状态查询响应：含文档当前术语版本与引用的全局术语库版本。 */

...
public class RaceRepository {
    ...
    /** 按全局退赛键查询退赛登记（用于 withdrawalKey 幂等重放）。 */
    ...
    /** 查询赛事下全部生效中（未撤销）的退赛登记。 */
    public List<WithdrawalRow> findActiveWithdrawals(String raceId) {
        return jdbcTemplate.query(
                "SELECT withdrawal_key, race_id, bib, status, reason, last_checkpoint_code, "
                        + "revoked, created_at, revoked_at "
                        + "FROM withdrawal WHERE race_id = ? AND revoked = FALSE "
                        + "ORDER BY created_at, withdrawal_key",
                WITHDRAWAL_ROW_MAPPER, raceId);
    }

    /** 查询某选手当前生效中的退赛登记；无则 empty。 */

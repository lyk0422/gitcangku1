...
public interface RaceService {
    ...
    /**
     * 退赛登记（DNS 未出发 / DNF 中途退赛）；成功后赛事版本加一。
     * 同状态重复登记幂等返回首次结果，DNS 与 DNF 相互改写返回409；
     * 已取消资格或已有完赛计时的选手登记返回409。
     */
    ServiceResult registerWithdrawal(String raceId, RegisterWithdrawalRequest request);

    /** 撤销退赛：携带同一 withdrawalKey 与当前赛事版本；已撤销的退赛再次撤销返回409。 */
    ServiceResult revokeWithdrawal(String raceId, String withdrawalKey, RevokeWithdrawalRequest request);

    /** 查询赛事退赛清单（含已撤销记录）；只读，不修改版本。 */
    WithdrawalListResponse getWithdrawals(String raceId);

    /** 查询单个选手的当前参赛状态；只读，不修改版本。 */

...
public class RaceServiceImpl implements RaceService {
    ...
    public ServiceResult registerRunner(String raceId, RegisterRunnerRequest request) {
        return withIdempotency(request.requestId(), "REGISTER_RUNNER",
                ...
                () -> {
                    ...
                    validateFinishTime(finishTimeMs);
                    long now = clock.millis();
    ...
    public ServiceResult reviseTime(String raceId, ReviseTimeRequest request) {
        return withIdempotency(request.requestId(), "REVISE_TIME",
                ...
                () -> {
                    ...
                    // 退赛后禁止提交完赛计时。
                    requireNoActiveWithdrawal(raceId, request.bib(), "提交完赛计时");
                    validateFinishTime(request.finishTimeMs());
                    // 修订后原始完赛耗时仍须严格大于该选手每一条已有分段耗时，否则分段不变量被破坏；
                    // finishTimeMs 为 null 表示清除完赛计时（回到 UNTIMED），既有分段记录保留。
                    if (request.finishTimeMs() != null) {
        ...
        return withIdempotency(request.requestId(), "ADD_PENALTY",
                ...
                () -> {
                    ...
                    if (type == PenaltyType.ADD_TIME) {
                        // 退赛后禁止处罚加时。
                        requireNoActiveWithdrawal(raceId, request.bib(), "处罚加时");
                        if (amountMs == null) {
    ...
    public ServiceResult submitTiming(String raceId, String bib, SubmitTimingRequest request) {
        return withIdempotency(request.requestId(), "SUBMIT_TIMING",
                ...
                () -> {
                    ...
                    // 退赛后禁止新增分段记录。
                    requireNoActiveWithdrawal(raceId, bib, "新增分段记录");
    ...
    public ServiceResult sealRace(String raceId, SealRaceRequest request) {
        return withIdempotency(request.requestId(), "SEAL_RACE",
                ...
                () -> {
                    ...
                    List<WithdrawalRow> withdrawals = repository.findWithdrawals(raceId);
    ...
    public StandingResponse getResults(String raceId) {
        ...
        return ResponseMapper.liveStanding(
                ...
                repository.findWithdrawals(raceId));
    }
    ...
    @Override
    @Transactional
    public ServiceResult registerWithdrawal(String raceId, RegisterWithdrawalRequest request) {
        return withIdempotency(request.requestId(), "REGISTER_WITHDRAWAL",
                orderedParams(
                        "raceId", raceId,
                        ...
                        "lastPassedCheckpoint", request.lastPassedCheckpoint(),
                        "expectedVersion", request.expectedVersion()),
                () -> {
                    // withdrawalKey 是第二层全局幂等键：同参重放原结果，异参409。
                    WithdrawalRow existing =
                            repository.findWithdrawal(request.withdrawalKey()).orElse(null);
                    if (existing != null) {
                        return replayWithdrawalOrConflict(raceId, request, existing);
                    }

                    RaceRow race = requireOpenRace(raceId, request.expectedVersion());
                    requireRunner(raceId, request.bib());
                    WithdrawalStatus status = parseWithdrawalStatus(request.status());
                    if (request.reason() == null || request.reason().isBlank()) {
                        throw new BadRequestException("退赛原因不能为空");
                    }
                    // 同选手已有生效退赛登记：同状态按幂等返回首次结果，DNS/DNF 相互改写409。
                    WithdrawalRow active =
                            repository.findActiveWithdrawal(raceId, request.bib()).orElse(null);
                    if (active != null) {
                        if (active.status() != status) {
                            throw new ConflictException(
                                    "DNS 与 DNF 不允许相互改写: " + request.bib());
                        }
                        return ServiceResult.created(ResponseMapper.toWithdrawalResponse(active));
                    }
                    // 已取消资格或已有完赛计时的选手禁止登记退赛。
                    boolean disqualified = repository.findPenalties(raceId).stream()
                            .anyMatch(penalty -> penalty.bib().equals(request.bib())
                                    && penalty.type() == PenaltyType.DISQUALIFY
                                    && !penalty.revoked());
                    if (disqualified) {
                        throw new ConflictException("选手已取消资格，禁止登记退赛: " + request.bib());
                    }
                    RunnerRow runner = repository.findRunner(raceId, request.bib()).orElseThrow();
                    if (runner.finishTimeMs() != null) {
                        throw new ConflictException("选手已有完赛计时，禁止登记退赛: " + request.bib());
                    }
                    List<CheckpointTimingRow> timings =
                            repository.findTimingsForRunner(raceId, request.bib());
                    String lastCheckpointCode =
                            validateWithdrawalPreconditions(status, request, timings);
                    long now = clock.millis();
                    bumpVersion(race, request.expectedVersion());
                    WithdrawalRow row = new WithdrawalRow(
                            request.withdrawalKey(), raceId, request.bib(), status,
                            request.reason(), lastCheckpointCode, false, now, null);
                    try {
                        repository.insertWithdrawal(row);
                    } catch (DuplicateKeyException ex) {
                        // 并发下主键兜底：同 withdrawalKey 走重放/异参冲突。
                        WithdrawalRow concurrent =
                                repository.findWithdrawal(request.withdrawalKey()).orElseThrow();
                        return replayWithdrawalOrConflict(raceId, request, concurrent);
                    }
                    WithdrawalRow saved =
                            repository.findWithdrawal(request.withdrawalKey()).orElseThrow();
    ...
    private ServiceResult replayWithdrawalOrConflict(
            String raceId,
            RegisterWithdrawalRequest request,
            WithdrawalRow existing) {
        boolean sameParams = existing.raceId().equals(raceId)
                && existing.bib().equals(request.bib())
                && existing.status().name().equals(request.status())
                && existing.reason().equals(request.reason())
                && Objects.equals(existing.lastCheckpointCode(), request.lastPassedCheckpoint());
        if (!sameParams) {
            throw new ConflictException(
    ...
    /**
     * 校验退赛前置条件并返回需固化的最后通过检查点代码。
     * DNS：不得携带 lastPassedCheckpoint，且该选手尚无任何分段记录；
     * DNF：必须携带 lastPassedCheckpoint，已有至少一条分段记录，
     * 且该检查点须为已有记录中顺序最大者，否则422。
     */
    private static String validateWithdrawalPreconditions(
            WithdrawalStatus status,
            RegisterWithdrawalRequest request,
            List<CheckpointTimingRow> timings) {
        if (status == WithdrawalStatus.DNS) {
            if (request.lastPassedCheckpoint() != null) {
                throw new BadRequestException("DNS 登记不得携带 lastPassedCheckpoint");
            }
            if (!timings.isEmpty()) {
                throw new UnprocessableEntityException(
                        "DNS 要求该选手尚无任何分段记录: " + request.bib());
            }
            return null;
        }
        if (request.lastPassedCheckpoint() == null || request.lastPassedCheckpoint().isBlank()) {
            throw new BadRequestException("DNF 登记必须携带 lastPassedCheckpoint");
        }
        if (timings.isEmpty()) {
            throw new UnprocessableEntityException(
                    "DNF 要求该选手已有至少一条分段记录: " + request.bib());
        }
        // timings 按 position 升序返回，最后一条即顺序最大的已通过检查点。
        CheckpointTimingRow last = timings.getLast();
        if (!last.checkpointCode().equals(request.lastPassedCheckpoint())) {
            throw new UnprocessableEntityException(
                    "lastPassedCheckpoint 须为该选手已有分段记录中顺序最大的检查点: "
                            + last.checkpointCode());
        }
        return last.checkpointCode();
    }
    ...
    public ServiceResult revokeWithdrawal(
        ...
        return withIdempotency(request.requestId(), "REVOKE_WITHDRAWAL",
                ...
                () -> {
                    RaceRow race = requireOpenRace(raceId, request.expectedVersion());
                    WithdrawalRow withdrawal = repository.findWithdrawal(withdrawalKey)
                            .orElseThrow(() -> new NotFoundException(
                                    "退赛登记不存在: " + withdrawalKey));
                    if (!withdrawal.raceId().equals(raceId)) {
                        throw new NotFoundException("退赛登记不属于该赛事: " + withdrawalKey);
                    }
                    if (withdrawal.revoked()) {
                        throw new ConflictException("退赛登记已撤销，不能再次撤销: " + withdrawalKey);
                    }
                    long now = clock.millis();
                    bumpVersion(race, request.expectedVersion());
                    int updated = repository.markWithdrawalRevoked(withdrawalKey, now);
                    if (updated == 0) {
                        throw new ConflictException("退赛登记已撤销，不能再次撤销: " + withdrawalKey);
                    }
                    WithdrawalRow refreshed = repository.findWithdrawal(withdrawalKey).orElseThrow();
    ...
    public WithdrawalListResponse getWithdrawals(String raceId) {
        ...
        return new WithdrawalListResponse(
                raceId,
                race.version(),
                repository.findWithdrawals(raceId).stream()
    ...
    public RunnerStatusResponse getRunnerStatus(String raceId, String bib) {
        ...
        RunnerRow runner = repository.findRunner(raceId, bib)
                .orElseThrow(() -> new NotFoundException("选手不存在: " + bib));
        WithdrawalRow latest = repository.findLatestWithdrawal(raceId, bib).orElse(null);
        var withdrawalResponse =
                latest == null ? null : ResponseMapper.toWithdrawalResponse(latest);
        if (race.status() == RaceStatus.SEALED) {
            SnapshotRow snapshot = repository.findSnapshot(raceId)
                    .orElseThrow(() -> new IllegalStateException(
                            "赛事已封榜但缺少快照: " + raceId));
            ...
            return new RunnerStatusResponse(
                    raceId, bib, entry.status(), entry.finishTimeMs(),
                    entry.lastCheckpointCode(), entry.missingCheckpoints(), withdrawalResponse);
        }
        // 以单选手视图复用即时成绩计算，保证状态口径与榜单一致。
        List<ResultEntry> entries = ResultCalculator.compute(
                List.of(runner),
                repository.findPenalties(raceId),
                repository.findCheckpoints(raceId),
    ...
    private static void validateFinishTime(Long finishTimeMs) {
    ...
    private static WithdrawalStatus parseWithdrawalStatus(String status) {
        try {
            return WithdrawalStatus.valueOf(status);
        } catch (IllegalArgumentException ex) {
            throw new BadRequestException("未知退赛状态: " + status);
        }
    }

    /** 退赛（生效中）选手禁止的写操作统一拦截：新增分段、提交完赛计时、处罚加时。 */
    private void requireNoActiveWithdrawal(String raceId, String bib, String operation) {
        if (repository.findActiveWithdrawal(raceId, bib).isPresent()) {
            throw new ConflictException("选手已登记退赛，禁止" + operation + ": " + bib);
        }
    }

...
import com.example.starter.domain.OptionalDecision;
import com.example.starter.domain.Platforms;
...
import com.example.starter.repo.RepositoryDao.LockOptionalRow;
...
public class ArtifactServiceImpl implements ArtifactService {
    ...
    public ArtifactResponse registerArtifact(String requestId, RegisterArtifactRequest request) {
        ...
        List<String> platforms = validateAndNormalizePlatforms(request);
        validateRegisterRequest(request);
        String hash = sha256(OP_REGISTER + "|" + request.name().trim() + "|" + request.version() + "|"
                + canonicalPlatforms(platforms) + "|" + canonicalDependencies(request));
        return executeIdempotent(requestId, OP_REGISTER, hash, 201,
                () -> doRegister(request, platforms), ArtifactResponse.class);
    }
    ...
    public LockFileResponse createLock(String requestId, LockRequest request) {
        ...
        String targetPlatform = validateTargetPlatform(request.targetPlatform());
        String hash = sha256(OP_LOCK + "|" + request.rootName().trim() + "|" + request.rootVersion()
                + "|" + request.expectedRepositoryVersion() + "|" + targetPlatform);
        return executeIdempotent(requestId, OP_LOCK, hash, 201,
                () -> doLock(request, targetPlatform), LockFileResponse.class);
    }
    ...
    public List<LockFileResponse> listLocks() {
        ...
        for (LockFileRow row : repositoryDao.listLockFiles()) {
            result.add(toLockResponse(row,
                    ...
                    repositoryDao.listLockOptional(row.id())));
        }
        return result;
    }
    ...
    public LockFileResponse getLock(long id) {
        ...
        return toLockResponse(row,
                ...
                repositoryDao.listLockOptional(id));
    }
    ...
    private ArtifactResponse doRegister(RegisterArtifactRequest request, List<String> platforms) {
        ...
        long artifactId = repositoryDao.insertArtifact(name, version, platforms, now);
        for (var dep : request.dependencies()) {
            repositoryDao.insertDependency(artifactId, dep.name().trim(),
                    dep.minimumVersion(), dep.maximumVersion(), dep.optional());
        }
        long repositoryVersion = repositoryDao.incrementRepositoryVersion();

        return new ArtifactResponse(name, version, platforms, false, repositoryVersion, now,
    ...
    private ArtifactResponse doWithdraw(String name, int version) {
        ...
        long repositoryVersion = repositoryDao.incrementRepositoryVersion();
        return new ArtifactResponse(name, version, artifact.platforms(), true, repositoryVersion,
                Instant.now(clock), toDependencyViews(artifact.dependencies()));
    }

    private LockFileResponse doLock(LockRequest request, String targetPlatform) {
        ...
        if (!root.supports(targetPlatform)) {
            // 平台不兼容的精确根：422，且不保存锁文件。
            throw ApiException.unprocessable("根制品 " + rootName + ":" + rootVersion
                    + " 不支持目标平台 " + targetPlatform);
        }

        RepositorySnapshot snapshot = repositoryDao.loadSnapshot();
        ResolutionResult resolution =
                LockResolver.resolve(snapshot, rootName, rootVersion, targetPlatform);
        if (resolution == null) {
            // 必选依赖无可行解：整体 422，不保存锁文件。仅可选依赖无解不会导致失败。
            throw ApiException.unprocessable(
                    "不存在满足全部必选依赖区间且兼容平台 " + targetPlatform
                            + " 的未撤回版本组合，无法锁定");
        }
        Map<String, Integer> solution = resolution.entries();

        Instant now = Instant.now(clock);
        long lockFileId = repositoryDao.insertLockFile(rootName, rootVersion, currentVersion,
                targetPlatform, currentRequestId.get(), now);
        solution.forEach((n, v) -> repositoryDao.insertLockEntry(lockFileId, n, v));
        for (OptionalDecision decision : resolution.optionalDecisions()) {
            repositoryDao.insertLockOptional(lockFileId, decision);
        }

        List<LockEntryResponse> entries = new ArrayList<>();
        solution.forEach((n, v) -> entries.add(new LockEntryResponse(n, v)));
        List<OptionalDependencyResponse> optionalViews = resolution.optionalDecisions().stream()
                .map(d -> new OptionalDependencyResponse(d.sourceName(), d.dependencyName(),
                        d.minimumVersion(), d.maximumVersion(), d.included(),
                        d.selectedVersion(), d.reason()))
                .toList();
        return new LockFileResponse(lockFileId, rootName, rootVersion, currentVersion,
                targetPlatform, now, entries, optionalViews);
    }
    ...
    /** 校验登记平台集合，失败转 400，返回规范化结果。 */
    private List<String> validateAndNormalizePlatforms(RegisterArtifactRequest request) {
        try {
            return Platforms.normalize(request.platforms());
        } catch (IllegalArgumentException e) {
            throw ApiException.badRequest(e.getMessage());
        }
    }

    /** 校验锁定目标平台：必须为具体 os/arch（不接受 ANY）。 */
    private String validateTargetPlatform(String raw) {
        if (raw == null || raw.isBlank()) {
            throw ApiException.badRequest("targetPlatform 不能为空");
        }
        String target = raw.trim();
        if (Platforms.ANY.equals(target) || !Platforms.isOsArch(target)) {
            throw ApiException.badRequest("targetPlatform 格式必须为 os/arch: " + raw);
        }
        return target;
    }

    private String canonicalPlatforms(List<String> platforms) {
        // 登记时平台顺序不影响语义，规范化为字典序参与幂等摘要。
        return platforms.stream().sorted().reduce((a, b) -> a + "," + b).orElse("");
    }
    ...
    private LockFileResponse toLockResponse(LockFileRow row, List<LockEntryRow> entries,
                                            List<LockOptionalRow> optionalRows) {
        List<LockEntryResponse> entryViews = entries.stream()
                .map(e -> new LockEntryResponse(e.name(), e.version()))
                .toList();
        // 迁移前的旧锁文件无 target_platform：新字段在响应中整体省略，保持旧查询结构不变。
        if (row.targetPlatform() == null) {
            return new LockFileResponse(row.id(), row.rootName(), row.rootVersion(),
                    row.repositoryVersion(), null, row.createdAt(), entryViews, null);
        }
        List<OptionalDependencyResponse> optionalViews = optionalRows.stream()
                .map(o -> new OptionalDependencyResponse(o.sourceName(), o.dependencyName(),
                        o.minimumVersion(), o.maximumVersion(), o.included(),
                        o.selectedVersion(), o.reason()))
                .toList();
        return new LockFileResponse(row.id(), row.rootName(), row.rootVersion(),
                row.repositoryVersion(), row.targetPlatform(), row.createdAt(),

...
def instance_paths(instance):
    ...
    fingerprint = hashlib.sha256(os.path.normcase(str(ROOT)).encode("utf-8")).hexdigest()[:12]
...
def compose_command(project, runtime, *arguments, capture=False, timeout=COMMAND_TIMEOUT_SECONDS):
    ...
    env = os.environ.copy()
    env["GSB_DB_PASSWORD"] = (runtime / "db-password").read_text(encoding="utf-8")
...
def health_url(project, runtime):
    binding = compose_command(project, runtime, "port", "app", "8080", capture=True)
